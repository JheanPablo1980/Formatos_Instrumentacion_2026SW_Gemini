...
export const VistaGenerar: React.FC = () => {
  const { instrumentos, perfiles, fotos, logoBase64 } = useAppStore();
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
  const [selectedProfile, setSelectedProfile] = useState('');
  const [isExporting, setIsExporting] = useState(false);

  const tagsConFotos = [...new Set(fotos.map(f => f.TAG))];
  const instrumentosConFotos = instrumentos.filter(inst => tagsConFotos.includes(inst.TAG));
  ...
  const handleSelectAll = () => {
    ...
    else setSelectedTags(instrumentosConFotos.map(i => i.TAG)); 
  };

  const exportarExcel = async () => {
    if (selectedTags.length === 0 || !activeProfile) { 
      alert("Selecciona al menos un instrumento y un perfil."); 
      return; 
    }
    setIsExporting(true);
    try {
      ...
      for (const tag of selectedTags) {
        const activeInstrument = instrumentos.find(i => i.TAG === tag);
        const fotosDelTag = fotos.filter(f => f.TAG === tag);
        ...
        ws1.mergeCells('C7:D7'); ws1.getCell('C7').value = activeInstrument.TAG; applyStyle(ws1.getCell('C7')); ws1.getCell('C7').alignment = {horizontal:'left'};
        ...
        ws1.mergeCells('A8:B8'); ws1.getCell('A8').value = 'Tipo / Descripción:'; applyStyle(ws1.getCell('A8'), true); ws1.getCell('A8').alignment = {horizontal:'left'};
        ws1.mergeCells('C8:H8'); ws1.getCell('C8').value = `${activeInstrument.TIPO_INSTRUMENTOS} / ${activeInstrument.DESCRIPCION}`; applyStyle(ws1.getCell('C8')); ws1.getCell('C8').alignment = {horizontal:'left'};
        ...
        ws1.mergeCells('A10:B10'); ws1.getCell('A10').value = 'Área / Sistema:'; applyStyle(ws1.getCell('A10'), true); ws1.getCell('A10').alignment = {horizontal:'left'};
        ws1.mergeCells('C10:D10'); ws1.getCell('C10').value = `${activeInstrument.AREA} / ${activeInstrument.SISTEMA}`; applyStyle(ws1.getCell('C10')); ws1.getCell('C10').alignment = {horizontal:'left'};
        ws1.mergeCells('E10:F10'); ws1.getCell('E10').value = 'Ref. Plano o P&ID:'; applyStyle(ws1.getCell('E10'), true); ws1.getCell('E10').alignment = {horizontal:'left'};
        ws1.mergeCells('G10:H10'); ws1.getCell('G10').value = activeInstrument.SIGNAL_ASSOCIATION || 'N/A'; applyStyle(ws1.getCell('G10')); ws1.getCell('G10').alignment = {horizontal:'left'};
  ...
  const exportarPDF = () => {
    if (selectedTags.length === 0 || !activeProfile) { 
        alert("Selecciona instrumentos y perfil."); 
        return; 
    }
        ...
        <div class="protocol-page">
          ...
          <table>
            ...
            <tr>
              ...
              <td class="bold uppercase" width="30%">${activeInstrument.TAG}</td>
              <td class="bg-gray" width="20%">Fabricante/Modelo:</td>
              <td width="30%">${activeProfile.FABRICANTE_MODELO || 'N/A'}</td>
            </tr>
            <tr>
              <td class="bg-gray">Tipo / Descripción:</td>
              <td colspan="3" class="uppercase">${activeInstrument.TIPO_INSTRUMENTOS} / ${activeInstrument.DESCRIPCION}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td class="bg-gray">Área / Sistema:</td>
              <td class="uppercase">${activeInstrument.AREA} / ${activeInstrument.SISTEMA}</td>
              <td class="bg-gray">Ref. Plano o P&ID:</td>
              <td class="uppercase">${activeInstrument.SIGNAL_ASSOCIATION || 'N/A'}</td>
            </tr>
          </table>
          ...
          ${fotosDelTag.length > 0 ? `
            <table><tr><td class="bg-blue" style="font-size:12px;">6. REGISTRO FOTOGRÁFICO</td></tr></table>
          ...
          <table style="margin-top: auto;">
            ...
            <tr>
              <td style="height: 60px; vertical-align: top; padding: 4px; position: relative;">
                <span class="bold" style="font-size: 8px; color: #666;">FIRMA:</span>
                ${activeProfile.ELABORO_FIRMA ? `<div style="text-align: center;"><img src="${activeProfile.ELABORO_FIRMA}" style="max-height: 40px; max-width: 90%; object-fit: contain;" /></div>` : ''}
              </td>
              <td style="height: 60px; vertical-align: top; padding: 4px; position: relative;">
                <span class="bold" style="font-size: 8px; color: #666;">FIRMA:</span>
                ${activeProfile.REVISO_FIRMA ? `<div style="text-align: center;"><img src="${activeProfile.REVISO_FIRMA}" style="max-height: 40px; max-width: 90%; object-fit: contain;" /></div>` : ''}
              </td>
              <td style="height: 60px; vertical-align: top; padding: 4px; position: relative;">
                <span class="bold" style="font-size: 8px; color: #666;">FIRMA:</span>
                ${activeProfile.APROBO_FIRMA ? `<div style="text-align: center;"><img src="${activeProfile.APROBO_FIRMA}" style="max-height: 40px; max-width: 90%; object-fit: contain;" /></div>` : ''}
              </td>
            </tr>
            <tr>
      ...
      <html>
        <head>
          ...
          <style>
            ...
            body { font-family: 'Inter', 'Helvetica', 'Arial', sans-serif; font-size: 9px; color: #000; margin: 0; padding: 0; }
            .protocol-page { width: 185mm; height: 270mm; margin: 10mm auto; padding: 5mm; border: 1px solid #eee; display: flex; flex-direction: column; }
            ...
            .photo-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 4px; margin-bottom: 6px; }
            .photo-cell { text-align: center; border: 1px solid #000; padding: 2px; }
            .photo-img { width: 100%; height: 180px; object-fit: contain; background: #fafafa; }
            .photo-caption { padding: 2px; font-size: 8px; border-top: 1px solid #000; }
        ...
        <div>
          ...
          <div className="w-full h-48 overflow-y-auto bg-gray-50 border border-gray-100 rounded-xl p-2 space-y-1 custom-scrollbar">
            {instrumentosConFotos.length === 0 ? (
              ...
              instrumentosConFotos.map(inst => (
                <label key={inst.TAG} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer border transition-all ${
                  selectedTags.includes(inst.TAG) 
                  ? 'bg-blue-50 border-blue-200' 
                  : 'hover:bg-white hover:border-gray-300 border-transparent'
                }`}>
                  <input 
                    type="checkbox" 
                    checked={selectedTags.includes(inst.TAG)}
                    onChange={() => handleToggleTag(inst.TAG)}
                    className="w-4 h-4 rounded border-gray-300 text-[#1F3864] focus:ring-[#1F3864]" 
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-[#1F3864] uppercase">{inst.TAG}</p>
                    <p className="text-[9px] text-gray-400 truncate uppercase font-medium">{inst.DESCRIPCION}</p>
                  </div>
                </label>
              ))
            )}
