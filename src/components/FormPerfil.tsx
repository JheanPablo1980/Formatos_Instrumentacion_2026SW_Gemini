...
export const FormPerfil: React.FC<FormPerfilProps> = ({ perfilToEdit, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await savePerfil({ ...formData, timestamp: new Date().toISOString() });
    onBack();
  };
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-6">
        ...
        <section className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InputGroup label="Fabricante / Modelo" name="FABRICANTE_MODELO" value={formData.FABRICANTE_MODELO} onChange={handleChange} />
            <InputGroup label="Rango de Operación" name="RANGO_OPERACION" value={formData.RANGO_OPERACION} onChange={handleChange} />
            <InputGroup label="Clase de Exactitud" name="CLASE_EXACTITUD" value={formData.CLASE_EXACTITUD} onChange={handleChange} />
          </div>
        </section>
        ...
        <div className="flex gap-4 pt-4">
          <Button type="button" onClick={handleClear} variant={confirmClear ? "danger" : "secondary"} icon={Trash2} className="flex-1">
            {confirmClear ? "Confirmar Limpiar" : "Limpiar Todo"}
          </Button>
          <Button type="submit" variant="primary" icon={Save} className="flex-[2]">Guardar Perfil</Button>
        </div>
      </form>
    </div>
  );
