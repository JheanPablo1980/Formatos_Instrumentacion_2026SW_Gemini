...
export const RegistroFotos: React.FC = () => {
  ...
  const fotosAPrevisualizar = fotos.filter(f => tagsParaPrevisualizar.includes(f.TAG));
  ...
  const handleToggleTag = (tag: string) => {
    setSelectedTags(prev => {
      if (prev.includes(tag)) {
        ...
        const fotosDelTag = fotos.filter(f => f.TAG === tag).length;
        if (fotosDelTag >= 4) {
          alert("Este TAG ya tiene el máximo de 4 fotos.");
          return prev;
        }
        return [...prev, tag];
      }
  ...
  const handleImageCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (tagsActualizados === 0) {
        alert("Verifica que los TAGs seleccionados no excedan el límite máximo de 4 fotos.");
      } else {
        setObservacion('');
        ...
        <div className="flex-1 overflow-y-auto border border-gray-200 rounded-lg bg-gray-50/30 p-1 space-y-0.5 custom-scrollbar">
          {filteredInstruments.length > 0 ? (
            filteredInstruments.map(inst => {
              const isSelected = selectedTags.includes(inst.TAG);
              const fotosDelInst = fotos.filter(f => f.TAG === inst.TAG).length;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-gray-500 truncate uppercase">{inst.DESCRIPCION || 'Sin descripción'}</div>
                  </div>
                  {fotosDelInst > 0 && (
              ...
              <div key={foto.id} className="relative bg-gray-50 p-2 rounded-xl border border-gray-200 animate-in zoom-in-95">
                <div className="aspect-square bg-gray-200 rounded-lg overflow-hidden mb-2 relative group">
                  ...
                  <button 
                    ...
                    className="absolute top-2 right-2 bg-white/90 p-1.5 rounded-full text-red-600 hover:bg-red-500 hover:text-white shadow-sm transition-colors"
                  >
                    <Trash2 size={16} />
                  </button>
